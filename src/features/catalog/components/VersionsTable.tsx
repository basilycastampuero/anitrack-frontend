...
export function VersionsTable({ versions }: VersionsTableProps) {
  ...
  return (
    ...
    >
      <div className="hidden grid-cols-6 gap-4 border-b border-border bg-muted/40 px-3 py-2 text-xs font-medium tracking-wide text-muted-foreground uppercase sm:grid">
        {HEADERS.map((header) => (
      ...
      <ul className="divide-y divide-border">
        {versions.map((version) => (
          <li
            key={version.id}
            className="grid grid-cols-2 gap-x-4 gap-y-2 px-3 py-3 sm:grid-cols-6 sm:items-center sm:gap-y-0 sm:py-2"
          >
            <Field label={t.detail.version.name} className="col-span-2 sm:col-span-1">
