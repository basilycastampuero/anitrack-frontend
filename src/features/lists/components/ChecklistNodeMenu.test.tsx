...
describe('ChecklistNodeMenu — rename', () => {
  it('renombra de forma optimista y cierra el dialog al confirmar el servidor', async () => {
    ...
    // Optimista: el árbol muestra el nombre nuevo y el dialog se cierra recién
    // cuando el servidor confirma. Este test no necesita latencia porque no
    // afirma "todavía no resolvió", solo el estado final de cada paso — a
    // diferencia de `useUpdateChecklist.test.tsx`, que sí la pone él mismo.
    await waitFor(() =>
      expect(
        screen.getByRole('treeitem', { name: 'Renamed live' }),
      ).toBeInTheDocument(),
    )
    await waitFor(() =>
...
describe('ChecklistNodeMenu — create', () => {
  it('crea una carpeta raíz desde el botón "New list" del header', async () => {
    ...
    expect(
      await screen.findByRole('treeitem', { name: 'Backlog (root)' }),
    ).toBeInTheDocument()
    await waitFor(() =>
      expect(screen.queryByRole('dialog')).not.toBeInTheDocument(),
    )
  ...
  it('crea una sub-lista bajo el nodo del menú', async () => {
    ...
    expect(
      await screen.findByRole('treeitem', { name: 'Currently airing' }),
    ).toBeInTheDocument()
  })
})
...
describe('ChecklistNodeMenu — delete', () => {
  it('muestra cuántas sub-listas se van con la cascada, y borra al confirmar', async () => {
    const parent = await listsService.createChecklist({
      name: 'Delete cascade parent',
    })
    await listsService.createChecklist({
      name: 'Delete cascade child',
      parentId: parent.id,
    })
    renderTree()
    ...
    expect(
      screen.getByText(
        'This will also delete 1 sub-list. This action cannot be undone.',
      ),
    ).toBeInTheDocument()
    ...
    await waitFor(() =>
      expect(
        screen.queryByRole('treeitem', { name: 'Delete cascade parent' }),
  ...
  it('un nodo hoja sin sub-listas ni entries muestra el mensaje de "lista vacía"', async () => {
    ...
    expect(
      await screen.findByText(
        'This list is empty. This action cannot be undone.',
      ),
    ).toBeInTheDocument()
    void user
  })
...
describe('ChecklistNodeMenu — teclado y accesibilidad', () => {
  ...
  it('las flechas dentro del menú abierto no mueven el foco roving-tabindex del árbol', async () => {
    renderTree()
    const user = await openMenuFor('Watching')
    const completed = await screen.findByRole('treeitem', {
      name: 'Completed',
      hidden: true,
    })
    ...
    expect(
      screen.getByRole('treeitem', { name: 'Watching', hidden: true }),
  ...
  it('Escape cierra el menú y devuelve el foco real de DOM al treeitem de origen', async () => {
    renderTree()
    const user = await openMenuFor('Watching')
    const watching = screen.getByRole('treeitem', {
      name: 'Watching',
      hidden: true,
    })
